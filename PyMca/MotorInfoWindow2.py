...
class MotorInfoHeader(qt.QHeaderView):
    ...
    def showEvent(self,  event):
        ...
        for i in range(1, self.count()):
            if not self.boxes[i]:
                newBox = MotorInfoComboBox(self,  self.parent().motorNamesList, i) # parent,  mlist,  nCol)
    ...
    def addLastSection(self):
        ...
        newBox = MotorInfoComboBox(self,  self.parent().motorNamesList, i) # parent,  mlist,  nCol)
        newBox.loadColumnSignal.connect(self.parent().loadColumn)
        newBox.setGeometry(self.sectionViewportPosition(i),  -1,  self.sectionSize(i) - 30 ,  self.height() )
        newBox.show()
        self.boxes += [newBox]
...
class MotorInfoTable(qt.QTableWidget):
    def __init__(self,  parent,  numRows,  numColumns,  legList, motList):
        ...
        for i in range(len(self.legendsList)):
            ...
            self.insertRow(i)
            self.setItem(i, 0,  self.textItem( curveLegend ))
            for j in range(1, self.columnCount()):
                self.setItem(0, j,  self.textItem( '' ))
...
class MotorInfoDialog(qt.QWidget):
    def __init__(self,  parent,  legends,  motorValues):
        ...
        self.buttonPerfom = qt.QPushButton("Perform action",  self)
        ...
        self.buttonLayout.addWidget(self.actionCBox )
        self.buttonLayout.addWidget(self.buttonPerfom)
...
#        self.buttonClose.clicked.connect(self.close)
        ...
        self.buttonPerfom.clicked.connect( self.performAction )
        # buttonUpdate is connected in MotorInfoPlugin
        self.updateActionList([('Invert selection',  self.table.invertSelection)])
