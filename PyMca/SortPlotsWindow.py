...
class SortPlotsScanWindow(qt.QWidget):
    ...
    def __init__(self,  origin=None,
        ...
        for legend in selection:
            ...
            if 'selectionlegend' in info:
                ...
                newLegend = leg
            self.scanWindow.addCurve(xVal,  yVal,  
                                                      newLegend, 
                                                      info)
        if activeCurve:
    ...
    def add(self):
        if DEBUG:
            print 'add():'
        activeCurve = self.scanWindow.getActiveCurve()
    ...
    def addAll(self):
        if DEBUG:
            print 'addAll():'
        for (xVal,  yVal,  legend,  info) in self.scanWindow.getAllCurves():
            if 'selectionlegend' in info:
                newLegend = info['selectionlegend']
            elif 'operation' in info:
                newLegend = (str(operation) + ' ' + self.title)
    ...
    def replaceAll(self):
        if DEBUG:
            print 'replaceAll()'
        allCurves = self.scanWindow.getAllCurves()
...
class SortPlotsTreeWidget(qt.QTreeWidget):
    def __init__(self,  parent):
       ...
       #self.header().setMaximumSectionSize(self.width()//2) #Only in version 5.2
...
class SortPlotsWidget(CloseEventNotifyingWidget.CloseEventNotifyingWidget):
    ...
    def getSelectedCurves(self):
        sel = []
        for elem in self.list.selectedItems():
            # Convert from QTreeWidgetItem to str
            sel += [str(elem.text(0))]
        print 'getSelectedCurves: %d Item(s) selected'%len(sel)
        return sel

    def removeCurve_(self):
        sel = self.getSelectedCurves()
        self.plotWindow.removeCurves(sel)
        self.updatePlots()

    def updatePlots(self,  newLegends = None,  newMotorValues = None):
        self._setLists()
        self.motorNamesList = [''] + self.getAllMotorNames()
        self.motorNamesList.sort()
        for cBox in self.cBoxList:
    ...
    def spawnNewScanWindow(self,  name=''):
        sel = self.getSelectedCurves()
        sw  = SortPlotsScanWindow(self.plotWindow, 
                                                    name, 
                                                    sel,
                                                    None)
        sw.plotModifiedSignal.connect(self.updatePlots)
        sw.show()
        sw.raise_()
        self.notifyCloseEventToWidget(sw)
        return sw
    ...
    def getActiveScanWindow(self):
        activeScanWindow = None
        for sw in [self.ScanWindowA,  self.ScanWindowB]:
            if sw:
                if sw.isActiveWindow():
                    activeScanWindow = sw
                    break
        return activeScanWindow
