...
#        pass
    ...
    def headerData(self,  section,  orientation,  role):
        ...
        '''
#        print "headerData -- Section: %s, Orientation: %d, Role: %s" % (str(section),  orientation,  str(role))
        if role != qt.Qt.DisplayRole:
            return qt.QVariant()
        if orientation == qt.Qt.Horizontal:
            return qt.QString( self.motorList[section] )
        if orientation == qt.Qt.Vertical:
            return qt.QString( self.legendList[section] )
...
class MyHorizontalHeader(qt.QHeaderView):
    def __init__(self,  parent):
        ...
        print 'New HorizHeader (count: %s)' % str(self.count())
        
    def showEvent(self,  event):
        print 'In MHH.showEvent.. count: %d\nBoxes: %s' % (self.count(), self.boxes.__repr__())
        if self.boxes == []:
            self.boxes = [None] * self.count()
        for i in range(self.count()):
            if not self.boxes[i]:
                self.boxes[i] = qt.QComboBox(self)
                self.boxes[i].addItems( [qt.QString(elem) for elem in ['foo', 'bar', 'baz']] )
            self.boxes[i].setGeometry(self.sectionViewportPosition(i),  -1,  self.sectionSize(i) - 30 ,  self.height() )
            self.boxes[i].show()
        qt.QHeaderView.showEvent(self, event)

    def handleSectionResized(self,  index):
        for i in range(self.visualIndex(index), self.count()):
            logical = self.logicalIndex (i)
#            if self.boxes != []:
            self.boxes[i].setGeometry(self.sectionViewportPosition(logical),  -1,  self.sectionSize(logical) - 30,  self.height())
#                box.setGeometry(self.sectionViewportPosition(logical),  -1,  self.sectionSize(logical) - 30,  self.height())

    def handleSectionMoved(self,  logical,  oldVisualIndex,  newVisualIndex):
        for i in range( qt.qMin(oldVisualIndex,  newVisualIndex),  self.count() ):
            logical = self.logicalIndex(i)
#            if self.boxes != []:
            self.boxes[i].setGeometry(self.sectionViewportPosition(logical),  -1,  self.sectionSize(logical) - 30,  self.height())
...
class MyTableView(qt.QTableView):
    ...
    def scrollContentsBy(self,  dx,  dy):
        '''
        Method goes into QTableview 
        '''
        qt.QTableView.scrollContentsBy(self,  dx,  dy )
        if (dx != 0):
...
if __name__ == '__main__':
    ...
    app
    table = MyTableView(None)
    table.resize(700,  500)
    sortmodel = qt.QSortFilterProxyModel(None)
    sortmodel.setSourceModel( a )
    table.setModel( sortmodel )
    print table.horizontalHeader().count()
    table.setHorizontalHeader( MyHorizontalHeader(table) )
